...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        // Else it is on, so let's make some magic happen
        	...
        	// Finally set the value of the ListView to the value of the mScannedDevices
        	// This is just a placeholder to make sure that the scan is behaving the correct way
	...
	public void onPlayClick (final View view)
	{
    	...
    	// Step through the list of common devices found in the two ArrayAdapters and save
    	...
    	for (int i = 0; i < mListUsers.getCount(); ++i) {
    		for (int j = 0; j < mScannedDevices.getCount(); ++j) {
    			// If the location in the mListUsers and mScannedDevices is a match, we have
    			// a player nearby
        		if (mListUsers.getItem(i).toString().equalsIgnoreCase(mScannedDevices.getItem(j).toString())) {
    	...
    	// This section looks up all the users in the mNearbyUsers_MAC and saves their username to
    	// be displayed to the user
    	for (int i = 0; i < mNearbyUsers_MAC.getCount(); ++ i) {
    		for (int j = 0; j < usersReturned.length(); ++j) {
    			try {
					if (mNearbyUsers_MAC.getItem(i).toString().equalsIgnoreCase(usersReturned.getJSONObject(j).get("m_strMac").toString()))
						mNearbyUsers_NAME.add(usersReturned.getJSONObject(j).get("m_strUsername").toString());
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
    	...
    	else
    	{
        	...
        	// Else alert the user there is no one around them
        		...
        		AlertDialog.Builder noPlayersFound = new AlertDialog.Builder(context);
        		// Set the title
        		noPlayersFound.setTitle("SocProx didn't find any players near you!");
        		// Set the message
        		noPlayersFound.setMessage("Click \"Try Again\" to scan one more time or \"Cancel\" to go back.");
        		// Set if the dialog box can be cancelled or not
        		noPlayersFound.setCancelable(false);
        		// Set what the positive button does
        		noPlayersFound.setPositiveButton("Try Again", new DialogInterface.OnClickListener()	{
        			public void onClick(DialogInterface dialog, int id) {
        				scanForPlayers();
        			}
        		});
        		// Set what the negative button does
        		noPlayersFound.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog,int id) {
